...
// New Schema for Bot Creation Limit
const BotLimitSchema = new mongoose.Schema({
  limit: { type: Number, default: 0 }, // Default to 0 (no limit) if not set
});
...
const setWebhook = async (token) => {
  const webhookUrl = `https://mybot-drab.vercel.app/created?token=${encodeURIComponent(token)}`;
  try {
...
makerBot.hears('🛠 Create Bot', async (ctx) => {
  ...
  try {
    ...
    const botLimit = botLimitDoc ? botLimitDoc.limit : 0; // Default to 0 (no limit) if not set
...
makerBot.command('clear', async (ctx) => {
  ...
  try {
    ...
    await BotLimit.deleteMany({}); // Clear bot limit as well
